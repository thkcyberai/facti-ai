...
class JWTService:
    ...
    def create_access_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
        ...
        Preserves 'type' if already set in data (e.g., for beta tokens)
        """
        to_encode = data.copy()
        if expires_delta:
    ...
    def verify_token(token: str, token_type: str = "access") -> Optional[Dict]:
        ...
        Accepts both 'access' and 'beta' tokens when token_type='access'
        """
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            
            # Check token type - allow "access" or "beta" for access tokens
            actual_type = payload.get("type")
            if token_type == "access" and actual_type not in ["access", "beta"]:
                return None
            elif token_type != "access" and actual_type != token_type:
                return None
